...
uint8_t backendMac[] = {0x08, 0xD1, 0xF9, 0xE7, 0x18, 0x2C}; // Backend MAC address
int backendPeerIndex = -1;
bool backendConnected = false;
unsigned long lastBackendCheckTime = 0;
#define BACKEND_CHECK_INTERVAL 15000 
...
void setup() {
  ...
  // ONLY connect to backend AFTER ESP-NOW is initialized
  if (deviceMode == MODE_A) {
    connectToBackend();
  }
  
  // Setup traffic light variables
...
void connectToBackend() {
  if (deviceMode != MODE_A) {
    return; // Only Mode A connects to backend
  }
  
  // Safety check - make sure ESP-NOW is initialized
  if (esp_now_init() != ESP_OK) {
    Serial.println("ESP-NOW not initialized, can't connect to backend");
    return;
  }
  ...
  // Try to find backend in peer list
  for (int i = 0; i < MAX_PEERS; i++) {
    if (peerList[i].active && memcmp(peerList[i].mac, backendMac, 6) == 0) {
      backendPeerIndex = i;
      backendConnected = true;
      importantLog("Backend found in peer list");
      return;
    }
  }
  
  // Add backend to peer list
  backendPeerIndex = addPeer(backendMac, NULL, 0);
  
  if (backendPeerIndex >= 0) {
    // Rename peer to identify as backend
    strcpy(peerList[backendPeerIndex].name, "Backend");
    backendConnected = true;
    importantLog("Backend added to peer list");
    
    // Send key exchange
    sendPublicKey(backendMac);
  } else {
    importantLog("Failed to add backend to peer list");
  }
}

// Send Mode B MAC address to backend
void reportModeBToBackend(const uint8_t* modeBMac) {
  if (!backendConnected || backendPeerIndex < 0) {
    importantLog("Backend not connected, can't report Mode B");
    return;
  }
  
  // Format MAC address as string
  char macStr[50];
  sprintf(macStr, "B:%02X:%02X:%02X:%02X:%02X:%02X", 
         modeBMac[0], modeBMac[1], modeBMac[2], 
         modeBMac[3], modeBMac[4], modeBMac[5]);
  
  // Save current selected peer
  int prevSelected = selectedPeerIndex;
  
  // Temporarily select backend
  selectedPeerIndex = backendPeerIndex;
  
  // Send message
  sendMessage(macStr, strlen(macStr));
  
  // Restore previous selection
  selectedPeerIndex = prevSelected;
  
  importantLog("Reported Mode B to backend");
}
...
void loop() {
  ...
  if (deviceMode == MODE_A && currentTime - lastBackendCheckTime >= BACKEND_CHECK_INTERVAL) {
    if (!backendConnected) {
      connectToBackend();
    }
    lastBackendCheckTime = currentTime;
  }
...
void updateDisplay(const char* message) {
  ...
  if (!displayInitialized) {
    if (!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDR)) {
      Serial.println("Display not initialized, can't update");
      return;
    }
...
void onDataReceived(const uint8_t* mac_addr, const uint8_t* data, int data_len) {
  ...
  if (data == NULL || data_len < sizeof(uint8_t)*2) {
  ...
  if (packet->length > data_len - sizeof(packet->type) - sizeof(packet->length)) {
    ...
    case DISCOVERY_BROADCAST:
      {
        if (deviceMode == MODE_A) {
          ...
          if (idx >= 0) {
            ...
            // Report this Mode B to the backend
            reportModeBToBackend(mac_addr);
          }
        }
      }
      break;
    ...
    case DATA_MESSAGE:
      {
        ...
        if (fromPeerIdx >= 0) {
          ...
          if (fromPeerIdx == backendPeerIndex) {
            // Parse the backend response
            if (strncmp(message, "OK ", 3) == 0) {
              int newRedTime, newBlueTime;
              if (sscanf(message, "OK %d %d", &newRedTime, &newBlueTime) == 2) {
                // Update timing values
                redLightDuration = newRedTime;
                blueLightDuration = newBlueTime;
                
                // Force switch to blue light
                digitalWrite(RED_LED, LOW);
                digitalWrite(BLUE_LED, HIGH);
                trafficLightStartTime = millis();
                isRedLight = false;
                ...
                char statusMsg[50];
                sprintf(statusMsg, "New times: R=%d B=%d", 
                      redLightDuration/1000, blueLightDuration/1000);
                importantLog(statusMsg);
                updateDisplay(statusMsg);
              }
            } 
            else if (strcmp(message, "NOT OK") == 0) {
              importantLog("Backend denied timing change");
              updateDisplay("Timing unchanged");
            }
          }
